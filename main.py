...
def predict():
    ...
    if 'file' not in request.files:
        print("Erreur: Aucun fichier envoyé")
    ...
    print(f"Fichier reçu: {file.filename}")  # Pour déboguer et vérifier le fichier reçu

    try:
        # Lire et préparer l'image uploadée
        img = Image.open(file.stream).convert('RGB')
        print("Image ouverte et convertie en RGB")
        
        # Vérifier la taille de l'image (utile pour le débogage)
        print(f"Dimensions de l'image avant redimensionnement: {img.size}")
        
        img = img.resize((224, 224))  # Tailles utilisées pour MobileNetV2
        print("Image redimensionnée à 224x224")

        img_array = np.array(img)
        img_array = np.expand_dims(img_array, axis=0)
        img_array = img_array / 255.0  # Normalisation

        # Vérifier la forme de l'image après transformation
        print(f"Forme de l'image après transformation: {img_array.shape}")
        ...
        # Chercher l'animal associé à la classe prédite
        ...
        if animal:
            ...
            print(f"Informations sur l'animal récupérées: {animal_info}")
        else:
            print("Aucun animal trouvé pour cette classe prédite")

        return jsonify(animal_info), 200

    except Exception as e:
        # Log de l'erreur pour mieux comprendre le problème
